...
def save_data(loc: str, data_name: str, fleet: pd.DataFrame, conv: List):
    """
    Saves the fleet data in the designated folder as a cvs file.
    NOTE: assumes that the save folder already exists!!!

    Inputs
    :param loc: path to the save folder
    :param data_name: name of the file where the fleet information will be stored
    :param fleet: pandas DataFrame that holds the fleet information
    :param conv: ongoing conversation list
    """
    # Construct the full fleet file name
    fleet_suffix = '_-_fleetdata.csv'
    fleet_path = os.path.join(loc, data_name+fleet_suffix)

    # Construct the full conversation file name
    conv_suffix = '_-_conversation.txt'
    conv_path = os.path.join(loc, data_name+conv_suffix)

    # Store the fleet information
    fleet.to_csv(fleet_path)

    # Write in the entire conversation with the customer
...
def get_basic_info(conv: List) -> Tuple[str, str, str, List]:
    """
    ...
    :return: name of the current user, ID/designation of the fleet, the base of file name that will be use for
             storing the data later on, and the ongoing conversation list
    """
    ...
    # Output the data
    return username, fleet_id, fn_base, conv
...
def get_single_truck(truck_nr: int, conv: List) -> Tuple[pd.Series, List]:
    ...
    criterion = lambda x: re.match(r'^\d$', x)
    err_msg = 'Please enter only a single whole number: '
...
def main():
    ...
    username, fleet_id, fn_base, conv = get_basic_info(conv)
    ...
    # Save the data
    save_data(loc=data_path, data_name=fn_base, fleet=fleet, conv=conv)
